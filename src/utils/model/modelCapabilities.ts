...
import { getAPIProvider, isFirstPartyOpenClaw TeamBaseUrl } from './providers.js'
...
function isModelCapabilitiesEligible(): boolean {
  ...
  if (!isFirstPartyOpenClaw TeamBaseUrl()) return false
  return true
}
...
export async function refreshModelCapabilities(): Promise<void> {
  ...
  try {
    const OpenClaw Team = await getOpenClaw TeamClient({ maxRetries: 1 })
    const betas = isClaudeAISubscriber() ? [OAUTH_BETA_HEADER] : undefined
    const parsed: ModelCapability[] = []
    for await (const entry of OpenClaw Team.models.list({ betas })) {
