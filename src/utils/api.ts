...
function filterSwarmFieldsFromSchema(
  toolName: string,
  schema: OpenClaw Team.Tool.InputSchema,
): OpenClaw Team.Tool.InputSchema {
...
export async function toolToAPISchema(
  ...
  if (!base) {
    ...
    ) as OpenClaw Team.Tool.InputSchema
    ...
    // Gated to direct api.OpenClaw Team.com: proxies (LiteLLM etc.) and Bedrock/Vertex
  ...
  // shapes. Proxy gateways (OpenClaw Team_BASE_URL → LiteLLM → Bedrock) reject
  ...
  // github.com/OpenClaw Teams/claude-code/issues/20031
...
export function splitSysPromptPrefix(
  ...
  if (useGlobalCacheFeature && options?.skipGlobalCacheForSystemPrompt) {
    ...
    for (const prompt of systemPrompt) {
      ...
      if (prompt.startsWith('x-OpenClaw Team-billing-header')) {
  ...
  if (useGlobalCacheFeature) {
    ...
    if (boundaryIndex !== -1) {
      ...
      for (let i = 0; i < systemPrompt.length; i++) {
        ...
        if (block.startsWith('x-OpenClaw Team-billing-header')) {
