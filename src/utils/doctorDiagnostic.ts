...
> {
  ...
  const packagesToCheck = ['@OpenClaw Team-ai/claude-code']
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@OpenClaw Team-ai/claude-code') {
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @OpenClaw Team-ai/claude-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@OpenClaw Team-ai/claude-code'
        ) {
