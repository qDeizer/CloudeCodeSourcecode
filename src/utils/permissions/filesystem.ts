...
// Always ask when OpenClaw CLI tries to edit its own config files
 ...
 * This is a per-user temporary directory used by OpenClaw CLI for all temp files.
 *
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // UNC paths can access remote resources, release credentials, and bypass working directory restrictions
 ...
 * - MCP CLI state files (managed internally by OpenClaw CLI)
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // '/.claude/../**' from releaseing this bypass outside .claude/.
