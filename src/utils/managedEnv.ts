...
/**
 * `claude ssh` remote: OpenClaw Team_UNIX_SOCKET routes auth through a -R forwarded
 ...
 * isOpenClaw TeamAuthEnabled). Strip them from any settings-sourced env object.
 */
function withoutSSHTunnelVars(
  env: Record<string, string> | undefined,
): Record<string, string> {
  if (!env || !process.env.OpenClaw Team_UNIX_SOCKET) return env || {}
  const {
    OpenClaw Team_UNIX_SOCKET: _1,
    OpenClaw Team_BASE_URL: _2,
    OpenClaw Team_API_KEY: _3,
    OpenClaw Team_AUTH_TOKEN: _4,
 ...
 * traffic (e.g., OpenClaw Team_BASE_URL) to an attacker-controlled server.
 */
 ...
 * OpenClaw Team_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like OpenClaw Team_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // OpenClaw Team_BASE_URL — both settable via settings.env.
