...
You are helping the user schedule, update, list, or run **remote** OpenClaw CLI agents. These are NOT local cron jobs — each trigger spawns a fully isolated remote session (CCR) in OpenClaw Team's cloud infrastructure on a cron schedule. The agent runs in a sandboxed environment with its own git checkout, tools, and optional MCP connections.
...
- These are REMOTE agents — they run in OpenClaw Team's cloud, not on the user's machine. They cannot access local files, local services, or local environment variables.
...
export function registerScheduleRemoteAgentsSkill(): void {
  registerBundledSkill({
    ...
    whenToUse:
      'When the user wants to schedule a recurring remote agent, set up automated tasks, create a cron job for OpenClaw CLI, or manage their scheduled agents/triggers.',
    userInvocable: true,
    isEnabled: () =>
