...
const GITHUB_ISSUES_REPO_URL = "external" === 'ant' ? 'https://github.com/OpenClaw Teams/claude-cli-internal/issues' : 'https://github.com/OpenClaw Teams/claude-code/issues';
type Props = {
...
export function redactSensitiveInfo(text: string): string {
  ...
  // OpenClaw Team API keys (sk-ant...) with or without quotes
      ...
      {step === 'consent' && <Box flexDirection="column">
          ...
          <Box marginTop={1}>
            <Text wrap="wrap" dimColor>
              ...
              OpenClaw CLI&apos;s functionality (eg. to reduce the risk of bugs
...
async function generateTitle(description: string, abortSignal: AbortSignal): Promise<string> {
  try {
    const response = await queryHaiku({
      systemPrompt: asSystemPrompt(['Generate a concise, technical issue title (max 80 chars) for a public GitHub issue based on this bug report for OpenClaw CLI.', 'OpenClaw CLI is an agentic coding CLI based on the OpenClaw Team API.', 'The title should:', '- Include the type of issue [Bug] or [Feature Request] as the first thing in the title', '- Be concise, specific and descriptive of the actual problem', '- Use technical terminology appropriate for a software issue', '- For error messages, extract the key error (e.g., "Missing Tool Result Block" rather than the full message)', '- Be direct and clear for developers to understand the problem', '- If you cannot determine a clear issue, use "Bug Report: [brief description]"', '- Any LLM API errors are from the OpenClaw Team API, not from any other model provider', 'Your response will be directly used as the title of the Github issue, and as such should not contain any other commentary or explaination', 'Examples of good titles include: "[Bug] Auto-Compact triggers to soon", "[Bug] OpenClaw Team API Error: Missing Tool Result Block", "[Bug] Error: Invalid Model Name for Opus"']),
...
async function submitFeedback(data: FeedbackData, signal?: AbortSignal): Promise<{
  ...
  try {
    ...
    const response = await axios.post('https://api.OpenClaw Team.com/api/claude_cli_feedback', {
    ...
    // Use our safe error logging function to avoid releaseing API keys
