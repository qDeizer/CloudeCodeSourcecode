 ...
 * OpenClaw Team OAuth token. Backed by the memoized getClaudeAIOAuthTokens —
...
export function hasVoiceAuth(): boolean {
  // Voice mode requires OpenClaw Team OAuth — it uses the voice_stream
  // endpoint on claude.ai which is not available with API keys,
  // Bedrock, Vertex, or Foundry.
  if (!isOpenClaw TeamAuthEnabled()) {
    return false
  }
  // isOpenClaw TeamAuthEnabled only checks the auth *provider*, not whether
