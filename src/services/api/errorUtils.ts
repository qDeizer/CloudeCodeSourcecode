 ...
 * The OpenClaw Team SDK wraps underlying errors in the `cause` property.
...
export function getSSLErrorHint(error: unknown): string | null {
  ...
  return `SSL certificate error (${details.code}). If you are behind a corporate proxy or TLS-intercepting firewall, set NODE_EXTRA_CA_CERTS to your CA bundle path, or ask IT to allowlist *.OpenClaw Team.com. Run /doctor for details.`
}
 ...
 * - Standard OpenClaw Team API: `{ error: { error: { message: "..." } } }`
 ...
 * 1. `error.error.error.message` — standard OpenClaw Team API shape
...
function extractNestedErrorMessage(error: APIError): string | null {
  ...
  // Standard OpenClaw Team API shape: { error: { error: { message } } }
